 ...
 * For example, if [prefix] is "PRE" and the field name is "myFieldName",
 * then the environment will look for a parameter named "PRE_MY_FIELD_NAME"
...
class PropertyDelegate<T>(
  ...
  private var initializer: (() -> T)?
                         ) : WithLogging by KLoggerHolder(), ReadWriteProperty<Any?, T> {
  ...
  override fun getValue(
    ...
    return value
           ?: run {
             val envName = (if (prefix.isNullOrEmpty()) property.name else "${prefix}_${property.name}").replace(
               REGEXP_ENV_NAME_UNDERSCORE, "_$1"
                                                                                                                )
               .toUpperCase()
             log.trace { "find environment variable '$envName'" }
             ...
             setValue(
               ...
               envValue
               ?: initializer?.invoke()
               ?: @Suppress("MaxLineLength") throw IllegalStateException("For the field '${property.name}' value has not been found - not found an environment variable '$envName' or default")
                     )
             return@run value!!
           }
  }
  ...
  override fun setValue(
    ...
    log.debug { "${property.name}=${if (security) "<PROTECTED DATA>" else value}" }
    this.value = value
  }
